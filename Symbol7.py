...
def Main():
    ...
    while Again == "y":
        ...
        if len(Filename) > 0:
            ...
            while True:
                difficulty_choice = input("Choose a difficulty, hard, medium, easy: ").lower()
                if difficulty_choice == "hard":
                    ...
                    print("Invalid difficulty optioin")
...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            self.__Grid = []    
            self.__difficulty = args[2]
            for Count in range(1, self.__GridSize * self.__GridSize + 1):
                if random.randrange(1, 101) < self.__difficulty:
                    C = Cell()
                else:
                    C = BlockedCell()
